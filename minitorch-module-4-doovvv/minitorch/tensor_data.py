...
def shape_broadcast(shape1: UserShape, shape2: UserShape) -> UserShape:
    ...
    shape1 = list(shape1)
    shape2 = list(shape2)
    # 规则2，3：只能在左侧填充大小为1的维度
    if len(shape1) < len(shape2):
        shape1 = [1] * (len(shape2) - len(shape1)) + shape1
    elif len(shape1) > len(shape2):
        shape2 = [1] * (len(shape1) - len(shape2)) + shape2 
    # 规则1：对应维度大小相同或其中一个为1，对为1的维度进行复制
...
def strides_from_shape(shape: UserShape) -> UserStrides:
    """
    此函数用给出的Shape计算Strides，Strides代表了在指定维度下由当前元素到下一个元素的步长。
    不难看出步长的每一个元素是有倍数关系的
    """
    layout = [1]
    offset = 1
...
class TensorData:
    ...
    def permute(self, *order: int) -> TensorData:
        ...
        # TODO: Implement for Task 2.1.
        return TensorData(self._storage,tuple([self.shape[x] for x in order]),tuple([self._strides[x] for x in order]))
        # raise NotImplementedError("Need to implement for Task 2.1")
